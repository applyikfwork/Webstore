    ...
    <div className="container mx-auto max-w-4xl px-4 py-12">
        <div className="grid md:grid-cols-[150px_1fr] gap-8 items-start">
            <div className="flex flex-col items-center gap-4">
                ...
                />
                 <div className="flex flex-col sm:flex-row md:flex-col gap-2 w-full pt-4">
                    {hasWebsite && (
                        <a 
                            href={app.websiteUrl}
                            target="_blank" 
            ...
            <div className="space-y-6">
                <div>
                    <h1 className="text-4xl font-bold font-headline">{app.name}</h1>
                    <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                             <CalendarDays className="h-4 w-4"/>
                            <span>Published on {app.createdAt?.toDate().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                        </div>
                        {(hasWebsite || hasApk) && <Badge variant="secondary" className="capitalize">
                            {hasWebsite && hasApk ? "Website & APK" : hasWebsite ? "Website" : "APK"}
                        </Badge>}
                    </div>
                </div>

                <div className="prose prose-invert max-w-none text-muted-foreground">
                    <h2 className="text-2xl font-semibold text-foreground border-b pb-2 mb-4">Description</h2>
                    <p>{app.description}</p>
                </div>
                
                <div className="prose prose-invert max-w-none text-muted-foreground">
                    <h2 className="text-2xl font-semibold text-foreground border-b pb-2 mb-4">Feature Highlights</h2>
